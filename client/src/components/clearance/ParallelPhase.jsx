  ...
  return (
    <Card className="p-6">
      <div className="space-y-4">
        {/* Header Block */}
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-slate-100 dark:border-slate-900 pb-3">
          <div className="flex items-center gap-3">
            <div className="p-2 rounded-xl bg-brand-parallel/15 text-brand-parallel">
              <Boxes size={20} />
            </div>
            <div>
              <h3 className="text-lg font-bold text-slate-950 dark:text-white font-display">
                Parallel Clearance Phases
              </h3>
              <p className="text-xs text-slate-500 dark:text-slate-400">
                These departments can be processed concurrently once unlocked.
              </p>
            </div>
          </div>

          {canSubmit && submissions.some((s) => s.status === "not_started" || s.status === "rejected") && (
            <Button
              onClick={onSubmitAll}
              variant="primary"
              className="rounded-xl bg-brand-parallel text-white hover:bg-brand-parallel/95 text-xs font-semibold py-2 px-4 shadow-md shadow-brand-parallel/10 w-full sm:w-auto self-start"
            >
        ...
        {/* Phase Unlock Reminder */}
        {!canSubmit && (
          <div className="p-3.5 rounded-xl bg-slate-50 dark:bg-slate-900/30 border border-slate-100 dark:border-slate-800 text-xs text-slate-500 dark:text-slate-400 flex items-start gap-2">
            <Lock size={15} className="shrink-0 mt-0.5" />
            <div>
              <span className="font-semibold text-slate-700 dark:text-slate-300">Parallel stage locked:</span> You must complete all stages in the Sequential Phase before you can upload documents for these departments.
            </div>
          </div>
        )}
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {submissions.map((s) => {
            ...
            if (!canSubmit) {
              ...
              borderStyle = "border-brand-parallel/30 dark:border-brand-parallel/10 bg-brand-parallel/[0.02] hover:border-brand-parallel/55 hover:bg-brand-parallel/[0.04]";
              statusBadge = (
                <span className="inline-flex items-center gap-1 text-[10px] font-bold uppercase tracking-wider text-brand-parallel bg-brand-parallel/10 px-2 py-0.5 rounded-md border border-brand-parallel/20">
                  Ready to Submit
                </span>
              );
              actionButton = (
                <Button
                  className="mt-3 w-full rounded-xl text-xs font-semibold py-2 bg-brand-parallel hover:bg-brand-parallel/95 text-white shadow-sm"
                  onClick={() => onSubmitSingle?.(s)}
                >
                  <UploadCloud size={14} className="mr-1.5" />
                  Upload Requirement
            ...
            return (
              <motion.div
                key={`par-${s.departmentId?._id || s.departmentId}`}
